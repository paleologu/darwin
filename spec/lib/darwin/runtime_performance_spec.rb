...
RSpec.describe Darwin::Runtime do
  describe 'performance' do
    it 'reloads the runtime in under 200ms with 4 models' do
      ...
      user_model.blocks.create!(method_name: 'attribute', args: %w[name string])

      profile_model = Darwin::Model.create!(name: 'Profile')
      profile_model.blocks.create!(method_name: 'attribute', args: %w[bio text])
      profile_model.blocks.create!(method_name: 'belongs_to', args: ['user'])

      post_model = Darwin::Model.create!(name: 'Post')
      post_model.blocks.create!(method_name: 'attribute', args: %w[title string])
      post_model.blocks.create!(method_name: 'attribute', args: %w[content text])
      post_model.blocks.create!(method_name: 'belongs_to', args: ['user'])

      comment_model = Darwin::Model.create!(name: 'Comment')
      comment_model.blocks.create!(method_name: 'attribute', args: %w[body text])
      comment_model.blocks.create!(method_name: 'belongs_to', args: ['post'])
      comment_model.blocks.create!(method_name: 'belongs_to', args: ['user'])
